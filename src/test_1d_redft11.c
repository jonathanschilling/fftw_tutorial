...
int test_1d_redft11(int n) {
    ...
    fftw_plan p = fftw_plan_r2r_1d(n, in, out, FFTW_REDFT11, FFTW_ESTIMATE);
    ...
    dump_1d_real("test_1d_redft11_in.dat", n, in);
    dump_1d_cplx("test_1d_redft11_in_logical.dat", N, in_logical);
    ...
    // use FFTW to execute REDFT11
    fftw_execute(p);

    dump_1d_real("test_1d_redft11_out.dat", n, out);
    dump_1d_cplx("test_1d_redft11_out_logical.dat", N, out_logical);
    ...
    // 3. odd symmetry of output values around n-0.5
    for (int i = 0; i < n; ++i) {
...
int main(int argc, char** argv) {
    int status = 0;
    status += test_1d_redft11(4);
    status += test_1d_redft11(5);
    return status;
}
