...
int test_1d_rodft11(int n) {
    ...
    fftw_plan p = fftw_plan_r2r_1d(n, in, out, FFTW_RODFT11, FFTW_ESTIMATE);
    ...
    dump_1d_real("test_1d_rodft11_in.dat", n, in);
    dump_1d_cplx("test_1d_rodft11_in_logical.dat", N, in_logical);
    ...
    dump_1d_real("test_1d_rodft11_out.dat", n, out);
    dump_1d_cplx("test_1d_rodft11_out_logical.dat", N, out_logical);
    ...
    // 2. next n values should have the output of RODFT11 in the negative imaginary part
    double delta;
    for (int i = 0; i < n; ++i) {
    ...
    // 3. even symmetry around (n-0.5) of neg. imag. part vs. output of RODFT11
    for (int i = 0; i < n; ++i) {
...
int main(int argc, char** argv) {
    int status = 0;
    status += test_1d_rodft11(4);
    status += test_1d_rodft11(5);
    return status;
}
