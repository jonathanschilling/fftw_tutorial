...
The reference output is computed now (before calling `fftw_execute`), since in some later examples (notably multi-dimensional `c2r` transforms),
...
Finally, the `fftw_plan` is destroyed and the arrays are freed using `fftw_free` (which has to be used if the array was allocated using `fftw_alloc_*`.
